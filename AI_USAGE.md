...
- 错位来自背景尺寸与子组件坐标基准不一致，不是拾取事件、物品缓存或 DragonCore 发包问题。
- 背景保持服主当前 `80 x 18` 尺寸，所有内容统一从背景左上角定位。
- 默认淡入为 `150ms`，默认层间距为 `22`；显示时长、淡出、最大条数和滑入逻辑保持不变。
- 旧服务器无需覆盖完整 `config.yml`：HUD 会把旧淡入 `0` 和旧层间距 `43` 转成新默认值。
- 本次不改物品品质匹配、XySoulSpace 来源判断、经验或金币数量。

验证记录：

- DragonCore 拾取 YML 与服务端默认配置均已通过 SnakeYAML 解析。
- `gradlew.bat clean test build --no-daemon` 已通过，共 11 项测试，失败 0、错误 0、跳过 0。
- JAR 内版本为 `1.3.12`，主类字节码为 Java 8（major 52）。
- 构建 SHA-256：`B5D3EC7E7A96B56C83FE472F21243B0F92D892980DB0B04D43E099E357346D99`。

## 1.3.11

本次修改配合 XySoulSpace 1.1.17 的MM掉落直投规则完成。

同时根据服主提供的新版 UI 文件完成拾取视图迭代。

已确认的实现边界：

- `战斗视图/拾取视图/拾取框.png` 是所有拾取来源共用的背景，不再维护普通/灵魂两套整张品质背景。
- HUD 只匹配 ItemEffect 前七项品质：白描、传神、浮世、极意、气象、青痕、石墨。
- 匹配品质后动态创建 `拾取品质框`，使用 `icon/品质框/*.gif` 并按 `30 x 30` 套入左侧物品格。
- 未匹配品质、经验和金币都不会创建品质框；它们仍正常显示统一背景、图标和文字。
- 新背景尺寸为 `169 x 41`，HUD 会把层间距限制为至少 `43`；旧服务器配置无需立刻补字段也不会再互相重叠。
- 玩家个人灵魂入库开启时，XYSS直接存入空间I并继续触发 `XySoulSpaceItemDepositEvent(source=pickup)`；本插件显示灵魂空间来源提示。
- 玩家个人灵魂入库关闭时，XYSS优先把MM掉落直接加入Bukkit背包并触发 `XySoulSpaceItemInventoryDeliveryEvent`；本插件显示普通来源提示。
- 新事件中的ItemStack数量只包含实际进入背包的部分。背包溢出量不会提前显示，余量落地后被真实捡起时再由 `PlayerPickupItemEvent` 显示。
- 两类XYSS事件分别注册独立反射监听，不把背包直投伪装成灵魂空间入库，也不要求XyBattleHud编译依赖XYSS。
- `pickup.soul-space-enabled` 继续只控制灵魂空间入库提示。即使关闭该项，只要总拾取视图开启，背包直投仍会显示。
- XySoulSpace低于1.1.17时找不到背包投递事件会静默跳过；原有灵魂入库桥和普通Bukkit拾取不受影响。
- 本次不增加地面扫描、定时任务、缓存或去重时间窗。

验证记录：

- `gradlew.bat clean test build --no-daemon` 已通过，共11项测试，失败0、错误0、跳过0。
- 产物为 `build/libs/XyBattleHud-1.3.11.jar`；JAR内 `plugin.yml` 为 `1.3.11`，主类与桥接类字节码均为Java 8（major 52）。
- 本次构建SHA-256为 `47A3F445C88691F2A781E1856B6B3D6ED05B6D2C27B432900DDDD15BCB0FB3CC`。
- 仍需实服复测灵魂入库开启、关闭且背包有空间、关闭且背包部分溢出三种场景。
