...
## 2026-09-03 - v1.3.14 避免拾取品质双框

- 根据服主提供的截图和 `ItemEffect.yml`，确认该文件的前七项已注明会同时作用于 HUD、背包和 ItemTip。
- 将 XyBattleHud 拾取视图内置品质框默认关闭，避免与 ItemEffect 对 `slot` 组件绘制的品质框叠在一起。
- 保留 `图片.品质框启用`、品质判断词和品质框路径，作为未使用 ItemEffect HUD 品质框时的备用方案。

## 2026-09-03 - v1.3.13 拾取内容独立尺寸

- 根据服主反馈确认 v1.3.12 的“随背景比例缩放”会让 `slot` 物品图标受到背景横纵比例影响，视觉上不再是正方形。
- 以服主提供的当前实服两份配置为基准，保留 `right: 100`、`stack-spacing: 30`、拾取框 `90 x 17` 和文字坐标方向。
- 将 DragonCore `布局` 段扩展为独立框内坐标：背景、品质框、物品、经验图标、金币图标和文字可以分别调整。
- 默认物品图标使用独立 `13 x 13`，经验和金币图标各自保留单独大小，后续微调只需改顶部中文字段。
- 同步服务端缺省值，避免旧 `config.yml` 缺少 `pickup.position.right` 或 `pickup.animation.stack-spacing` 时回落到旧位置。

## 2026-09-02 - v1.3.12 拾取组件统一锚点

- 根据服主实服截图定位坐标错位原因：背景已被调为 `80 x 18`，子组件仍写死旧版 `169 x 41` 基准。
- 在 DragonCore YML 顶部新增两个布局尺寸，背景与所有子组件统一读取，不增加服务端事件或缓存。
- 子组件偏移和大小按原始 `169 x 41` 设计比例自动换算，后续调整背景尺寸时仍保持套在框内。
- `取透明度` 补上此前未实际执行的淡入分支；默认淡入改为 `150ms`。
- 移除强制最小 `43` 层间距，默认改为适合当前尺寸的 `22`。
- 为已有服务器配置增加轻量迁移：淡入 `0` 转为 `150`，层间距 `43` 转为 `22`；淡入 `-1` 明确表示关闭。

## 2026-09-01 - v1.3.11 拾取视图与XYSS背包直投

- 根据服主提供的新版 `ItemEffect.yml`、服务器拾取视图和客户端 `拾取框.png`，确认新背景为 `169 x 41`，左侧品质框按 `30 x 30` 叠加。
- 将旧版“按品质替换整张普通/灵魂拾取框”改为“统一背景 + 独立品质框组件”。
- 品质匹配只采用 ItemEffect 注释明确说明会作用于 HUD 的前七项；无品质时不创建品质框组件。
- 同步重排物品、经验、金币和文字坐标，并保留服主微调的金币 `15 x 15` 独立尺寸与滑入系数。
- 将 `stack-spacing` 默认值和 HUD 最低值改为 `43`，适配新版背景高度。
- 配合XySoulSpace 1.1.17新增反射监听 `XySoulSpaceItemInventoryDeliveryEvent`。
- 将背包直投显示为普通来源，数量采用事件中已经扣除背包溢出后的真实入包数量。
- 保留 `XySoulSpaceItemDepositEvent(source=pickup)` 的灵魂空间来源显示，两类投递不会互相重复。
- 将灵魂入库事件和背包投递事件改为分别可选注册，旧XYSS缺少新事件类时保持兼容。
- `pickup.soul-space-enabled` 关闭时只跳过灵魂入库事件，仍注册背包直投事件并显示统一背景。
- 未增加XYSS编译依赖、常驻任务、扫描或额外缓存。
- 同步更新README、CHANGELOG、版本号和AI使用记录。
...
- 直接查看客户端资源图片，确认 `萌黄拾取框.png` 是头像框，`萌黄灵魂拾取框.png` 是箱子框；基础白描两张则仍是文件名与视觉相反。
- 修正 DragonCore 选择逻辑：普通来源命中品质时使用 `图片.品质普通拾取框`，灵魂来源命中品质时使用 `图片.品质灵魂拾取框`。
- 不改服务端 Java 逻辑，不改变拾取队列、金币/经验文本模式、位置和动画参数。

## 2026-08-18 - v1.3.7 金币图标独立组件

- 根据服主反馈“金币的大小和在拾取框里的坐标需要单独调”，将 DragonCore 文本提示图标拆分为经验与金币两套组件。
- `money` 来源现在创建 `拾取金币图标`，`experience` 来源继续创建 `拾取经验图标`。
- 采用服主提供的微调版 `config.yml`，默认 `pickup.position.right` 改为 `50`。
- 采用服主提供的微调版拾取视图参数，经验图标保留 `13 x 12`、`+4/+4` 和 `1.05` 滑入系数；金币图标初始使用同一组参数，后续可独立修改。
