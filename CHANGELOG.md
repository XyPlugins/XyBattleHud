...
- DragonCore 拾取视图将背景尺寸与框内内容尺寸拆开，改 `拾取框宽度/拾取框高度` 不再拉伸物品、经验或金币图标。
- 新增 `布局.物品X/Y/宽度/高度`、`经验图标X/Y/宽度/高度`、`金币图标X/Y/宽度/高度`、`品质框X/Y/宽度/高度` 和 `文字X/Y/文字大小`，方便直接微调。
- 默认物品图标改为独立 `13 x 13`，避免因背景横纵缩放比例不同而显示成长方形。
- 服务端代码缺省值同步为 `right: 100` 和 `stack-spacing: 30`，旧配置漏写字段时也会使用新默认位置。
